 ...
 * 2021-04-07 21h00 1st time a build succeeded partly, with a fatal error about #include <tr1/unordered_map>, so commented-it out
...
#include "../../../pico-sdk/src/common/pico_base/include/pico/types.h" // at the end of this file: typedef sturct of datetime_th"
...
bool a_itm_handler()
{
  ...
  if (pico_explorer.is_pressed(pico_explorer.X))
  {
    ...
    btns = btns | 0x4;
    std::cout << "btns: " << std::bitset<4>(btns) << std::endl;
    if (menu == MENU_MAIN)
      btn_xy_itm(true);
    else if (menu == MENU_SETTINGS)
...
int a_menu_handler()
{
  switch(menu)  // global menu defined in app_v2.hpp
  {
    case MENU_MAIN:
      a_itm_handler();
      break;
...
void btn_xy_settings(bool dr)
{
  ...
  {
    sett_itm -= 1;
    if (sett_itm < SETT_LED)  // We don't handle 0 (i.e. seconds)
      sett_itm = SETT_BACK; 
  }
  else if (dr == true)  // Next
  {
...
void fail_msgs(uint8_t code)
{
  ...
  {
    case 0x0:
      s2 = "";  // this is a dummy entry to correct
      break;
    case 0x1:
      s2 = s.at(SU_RTC_INIT)+s.at(SU_FAILED);
      break;
    case 0x2:
      s2 = s.at(SU_RTC_SETUP)+s.at(SU_FAILED);
      break;
    case 0x4:
      s2 = s4+s.at(SU_REST_FM_INIT_LED)+s.at(SU_FAILED);
      break;
    case 0x8:
      s2 = s4+s.at(SU_REST_FM_INIT_LED)+" and " +s.at(SU_REST_FM_INIT_BF)+s.at(SU_FAILED);
      break;
    case 0x10:
      s2 = s4+s.at(SU_FAILED);
      break;
    case 0x20:
      s2 = s.at(SU_RTC_UPDATETIME)+s.at(SU_FAILED);
      break;
    case 0x40:
...
void set_DispColour(bool bf, int disp_colour, bool cleardisp)
{
  ...
  */
  ...
  {
    case DISP_BLACK:
       pico_explorer.set_pen(disp_black[0], disp_black[1], disp_black[2]);
       break;
    case DISP_RED:
      pico_explorer.set_pen(disp_red[0], disp_red[1], disp_red[2]);
      break;
    case DISP_ORANGE:
      pico_explorer.set_pen(disp_orange[0], disp_orange[1], disp_orange[2]);
      break;
    case DISP_YELLOW:
      pico_explorer.set_pen(disp_yellow[0], disp_yellow[1], disp_yellow[2]);
      break;
    case DISP_GREEN:
      pico_explorer.set_pen(disp_green[0], disp_green[1], disp_green[2]);
      break;
    case DISP_BLUE:
      pico_explorer.set_pen(disp_blue[0], disp_blue[1], disp_blue[2]);
      break;
    case DISP_AUBERGINE:
      pico_explorer.set_pen(disp_auber[0], disp_auber[1], disp_auber[2]);
      break;
    case DISP_WHITE:
      pico_explorer.set_pen(disp_white[0], disp_white[1], disp_white[2]);
      break;
    default:
      pico_explorer.set_pen(disp_green[0], disp_green[1], disp_green[2]);
      break;
...
int w_btn()
{
  ...
  for (auto i = 0; i < 4; i++)
  {
    ...
    {
      n = i;
      btns = btns ^ idx;  // Clear the button
      break;
    }
  }
  std::cout << "button " << n << " pressed." << std::endl;
  return n;
}
